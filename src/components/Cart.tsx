      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex flex-col">
            <h2 className="text-lg font-bold text-primary">Cart</h2>
            {cartItems.length > 0 && (
              <button
                onClick={onClearCart}
                className="text-secondary hover:text-secondary/80 text-xs font-medium transition-colors underline"
              >
                Clear all
              </button>
            )}
          ...
          {onCloseCart && (
            <button
              onClick={onCloseCart}
              className="bg-accent text-white p-2 rounded-lg hover:bg-accent/90 transition-all duration-200 text-sm"
            >
              <CloseIcon size={14} />
            </button>
          )}
        ...
        {cartItems.length === 0 ? (
          ...
          <>
                  ...
                  <div key={item.card.id} className="flex flex-col items-start">
                    <div className="flex justify-between w-full items-start">
                      <div className="flex items-start space-x-2 mb-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <h4 className="text-primary font-medium text-xs truncate">
                            {item.card.name}
                          </h4>
                    ...
                    <div className="grid grid-cols-12 items-center justify-start w-full gap-2 mb-4">
                      <button
                        ...
                        className="col-span-2 h-10 bg-gray-600 border border-gray-500 text-gray-300 rounded flex items-center justify-center hover:bg-gray-500 transition-colors"
                      >
                        <MinusIcon size={16} />
                      </button>

                      <span className="col-span-8 text-primary font-semibold text-base text-center bg-gray-600 border border-gray-500 rounded py-2 px-2">
                        {item.quantity}
                      </span>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex justify-between items-center">
                <span className="text-secondary text-xs">Total price:</span>
                <span className="text-white font-semibold text-sm">
                  ${totalPrice.toFixed(2)}
                </span>
              </div>

              <button className="w-full bg-accent text-white py-3 px-4 rounded-lg font-semibold hover:bg-accent/90 transition-all duration-200 text-sm">
                Continue to Payment
              </button>
            </div>
          </>
